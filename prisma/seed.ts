import { PrismaClient, Sport, UserRole } from "@prisma/client";

import {
  DEFAULT_EXPECTED_TOTAL_PLAYERS_PER_OWNER,
...
async function main() {
  ...
  await prisma.leagueSettings.create({
    data: {
      expectedTotalPlayersPerOwner: DEFAULT_EXPECTED_TOTAL_PLAYERS_PER_OWNER,
  ...
  const keeperDefinitions: Array<{ round: number; ownerName: (typeof OWNER_NAMES)[number]; playerName: string; sport: Sport; tag: string }> = [
  ...
  for (const keeperDefinition of keeperDefinitions) {
    ...
    await prisma.draftSlot.update({
      ...
      data: {
        ...
        isKeeper: true,
        selectedAt: new Date(),
        originalRawValue: `(${owner?.code}) ${keeperDefinition.playerName}`,
      },
    });
  ...
  const samplePlayers: Array<{ playerName: string; sport: Sport }> = [
